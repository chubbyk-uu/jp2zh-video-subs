...
Windows 用户可从
[最新 Release](https://github.com/chubbyk-uu/jp2zh-video-subs/releases/latest)
...
- 可从“管理模型…”打开模态下载窗口：选择当前配置或全部缺失模型，切换
  ...
  界面显示本次会话的平均速度和可展开详情；已安装模型可手动勾选后重新下载，或经二次
...
`compat` 对应取消勾选后的兼容 HTTP 模式。GUI 和项目下载器都从统一目录读取经过筛选的
文件清单及固定 upstream revision；远端 metadata 解析不到该 revision 时会在传输前失败。
