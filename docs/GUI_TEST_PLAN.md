...
### RUN-20260727-22：v0.1.0 发布前 Anime 批处理验收

| 项目 | 记录值 |
|---|---|
| 自动测试 | PASS：`477 passed`；Ruff 通过 |
| Anime 文本批处理 | PASS：所选 ASR 批大小同时用于 Anime 文本生成；显存不足时自动拆分批次并保持输出顺序 |
| A/B 识别语义 | PASS：79 个真实片段在批大小 1/4/8/16/24 下未出现词语变化；仅观察到一处标点差异 |
| Windows 完整流程 | PASS（人工）：用户在最新 Windows portable package 上完整处理一次视频，流程与输出正常 |
| 补充硬件证据 | PASS（人工、用户报告）：此前 portable package 已在 12 GB RTX 4080 Laptop + i7-1165G7 环境完成实际任务；弱 CPU 下较大批次可能让 GPU 等待供数，需按实测速度降低批次 |
| 发布边界 | 英文翻译继续标注为实验性并建议人工复核；其它 NVIDIA GPU/驱动组合仍未逐一验证 |

### RUN-20260727-23：v0.1.1-dev OOM 与模型下载状态修复

| 项目 | 记录值 |
|---|---|
| 自动测试 | PASS：`480 passed`；Ruff、Python 编译、Shell 语法和 `git diff --check` 通过 |
